...
class BillingDoCitizenHttpServiceHelper(models.AbstractModel):
    _name = "billing.do.citizen.http.service.helper"
    _inherit = "billing.do.authentication.http.service.helper"
    _description = """
    
    """

    def dgii_get_citizen_info(self, vat):
        config = self.env['ir.config_parameter'].sudo()

        log.info("[KCS] Service Citizens Switch: {0}".format(config.get_param('billing_do.api_services_tax_contributors_switch')))
        ...
        if config.get_param('billing_do.api_services_tax_contributors_switch'):
            ...
            log.info("[KCS] Service Citizens Endpoint: {0}".format(config.get_param('billing_do.api_services_citizens_endpoint')))
            ...
            response = self.send_request("{0}/{1}/{2}".format(config.get_param('billing_do.api_services_base_url'),
                                                            config.get_param('billing_do.api_services_citizens_endpoint'),
                                                            vat),
