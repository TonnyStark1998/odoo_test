...
# Check if the DB_NAME environment variable was set, if not this must fail the start of the container.
if [[ -z ${DB_NAME} ]]; then
    echo "[$(date '+%Y-%m-%d %H:%M:%S.%N')][entrypoint.sh] Database name was not specified."
    echo "Use the environment variable $DB_NAME to specify the name of the database to use for this container."
    exit 1
fi
...
if [[ -n $USE_DEFAULT_ADDONS_PATH && "${USE_DEFAULT_ADDONS_PATH,,}" == "y" ]]; then
...
if [[ -e /mnt/extra-addons/$ODOO_VERSION/client-specific/$DB_NAME
    && -d /mnt/extra-addons/$ODOO_VERSION/client-specific/$DB_NAME ]]; then
    ODOO_ARGS="$ODOO_ARGS,/mnt/extra-addons/${ODOO_VERSION}/client-specific/${DB_NAME}"
fi
